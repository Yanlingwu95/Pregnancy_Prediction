...
def VM_download_file(myindex, filename):
    try:
        # Connect to your hub.
        iothub_device_method = IoTHubDeviceMethod(CONNECTION_STRING)

        # Call the direct method.
        print("type: {}".format(type(filename)))
        response = iothub_device_method.invoke(DEVICE_ID[myindex], "make_classify", filename, TIMEOUT)
...
def askfordata(myindex):
    filename = iothub_device_askdata(myindex)
...
if __name__ == '__main__':
    ...
    for i in range (50): #while true
        for j in range(6):  #
            ...
            if isconnect:
                print ("success")
                filename = askfordata(sendpointer.val)
                swap(myqueqq)
                sendpointer = myqueqq.head
                time.sleep(5)
                isconnect = VM_download_file(4,filename)
                break
            else:
                ...
                if ti == 2:
                    ...
                    message = "pi " + str(sendpointer.val) + " is extremely destoryed!!!!!! \n You need to check it!!"
                    sendmail.sendemail(message)
                    print("Send an email to farmer -- broken pi inromation -- successfully!!!")
                swap(myqueqq)
                sendpointer = myqueqq.head
            time.sleep(2)
        time.sleep(20)
